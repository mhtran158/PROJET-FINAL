...
colonnes_selectionnees = ['Poids (kg)', 'Cylindrée (cm3)', 'Puissance (ch)', 'Dimension (mm)', 'Autonomie electrique (km)'] 

# Création d'un nouveau DataFrame avec les colonnes sélectionnées
data_heatmap = data_target13.loc[:, colonnes_selectionnees]
...
X_train_reduced = X_train_new[['Poids (kg)','Autonomie electrique (km)','Dimension (mm)','Puissance (ch)','Cylindrée (cm3)']].copy()
X_test_reduced = X_test_new[['Poids (kg)','Autonomie electrique (km)','Dimension (mm)','Puissance (ch)','Cylindrée (cm3)']].copy()
...
regressor_decision_tree.fit(X_train_new[['Poids (kg)','Autonomie electrique (km)','Dimension (mm)','Puissance (ch)','Cylindrée (cm3)']], y_train)

fig, ax = plt.subplots(figsize=(20, 20))  

plot_tree(regressor_decision_tree, 
          feature_names = ['Poids (kg)','Autonomie electrique (km)','Dimension (mm)','Puissance (ch)',"Cylindrée (cm3)"], 
          filled = True, 
          rounded = True)
...
X_train_unscaled_reduce = X_train_unscaled[['Poids (kg)','Autonomie electrique (km)','Dimension (mm)','Puissance (ch)',"Cylindrée (cm3)"]]
...
y_pred_test = random_forest.predict(X_test_new)    


#Score de Random Forest:
print('Score du modèle RF sur train:', random_forest.score(X_train_new,y_train))
print('Score du modèle RF sur test:', random_forest.score(X_test_new,y_test))


y_pred_random_forest_train = random_forest.predict(X_train_new)

#Features importances du modèle de Random Forest Regressor:
import matplotlib.pyplot as plt

feat_importances = pd.DataFrame(random_forest.feature_importances_, index=X_train_new.columns, columns=["Importance"])
feat_importances.sort_values(by='Importance', ascending=False, inplace=True)
feat_importances.plot(kind='bar', figsize=(8,6))
plt.show()
...
mae_random_forest_test = mean_absolute_error(y_test,y_pred_test)
mse_random_forest_test = mean_squared_error(y_test,y_pred_test,squared=True)
rmse_random_forest_test = mean_squared_error(y_test,y_pred_test,squared=False)
